...
const mcUrl = (relative) =>
  url.resolve(platformBaseUrl, relative);
...
function parseChallengeInitialResponse(response)
{
  ...
  response = ini.parse(response.replace(/<br\/>/g, '\n'));
...
async function beginChallenge(problemId)
{
  const response = await request
  ({
    method: 'post',
  ...
  const result =
  {
    ...
    inputs: parseChallengeInitialResponse(response.data)
  };
...
module.exports.solve = async (problemId, solve) =>
{
  assert(/^(?:string|number)$/.test(typeof problemId));
  assert(solve instanceof Function);

  const challenge = await beginChallenge(problemId),
        solution  = await solve(challenge.inputs),
        result    = await endChallenge(challenge, solution);
  ...
  if(result.success || result.already || result.points)
  {
    const msg = [];

    result.success && msg.push('Correct!');
